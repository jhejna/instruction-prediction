...
class BehaviorCloningForwardPrediction(object):
    ...
    def __init__(self, env, network_class, network_kwargs={}, device="cpu",
        ...
        self.forward_criterion = torch.nn.CrossEntropyLoss(ignore_index=self.network.action_pad_idx)
    ...
    def load(self, checkpoint, strict=True):
        print("[Forward Prediction' Loading Checkpoint:", checkpoint)
    ...
    def _compute_loss(self, obs, actions):
        ...
        action_logits, forward_logits, anchor_logits = self.network(obs, forward_inputs=forward_inputs, is_target=False)
        
        if self.unsup_coeff > 0: # Note that we run unsupervised losses first to avoid action reshaping issues.
            with torch.no_grad():
                _, _, target_logits = self.ema_network(obs, forward_inputs=None, is_target=True)

...
class Reptile(object):
    ...
    def __init__(self, env, network_class, network_kwargs={}, device="cpu",
        ...
        if isinstance(self.env, gym.Wrapper) and isinstance(self.env.unwrapped, MazebaseGame):
    ...
    def train(self, path, total_steps, log_freq=10, eval_freq=100, eval_ep=0, validation_metric="adapt/action_loss", use_eval_mode=True, workers=4):        
        ...
        if isinstance(self.env, str) and self.env == 'omniglot':
            # Add support for omniglot.
            from language_prediction.datasets import omniglot
            dataset = omniglot.OmniglotDataset(self.num_support, self.num_query)
            collate_fn = omniglot.identity
        ...
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=self.meta_batch_size, shuffle=(sampler is None), num_workers=workers, pin_memory=True, collate_fn=collate_fn, sampler=sampler)
        if not self.validation_dataset is None:

...
class Subgoal:
    ...
    def __init__(self, bot=None, datum=None, reason=None):
        ...
        self.id = self.bot.subgoal_counter
        self.bot.subgoal_counter += 1
    ...
    def as_language(self):
        """ To get it as langauge instructions """
...
LanguageObj = namedtuple('LanguageSubgoal', ['text', 'obj_type', 'obj_color', 'subgoal_type', 'id', 'pos'])
...
class CloseSubgoal(Subgoal):
    ...
    def as_language(self):
        text = "close the " + self.fwd_cell.color + " " + self.fwd_cell.type
        return LanguageObj(text=text, obj_type=self.fwd_cell.type, 
                           obj_color=self.fwd_cell.color, subgoal_type=type(self), 
                           id=self.id, pos=tuple(self.fwd_pos))
...
class OpenSubgoal(Subgoal):
    ...
    def as_language(self):
        text = "open the " + self.fwd_cell.color + " door"
        return LanguageObj(text=text, obj_type=self.fwd_cell.type, 
                           obj_color=self.fwd_cell.color, subgoal_type=type(self), 
                           id=self.id, pos=tuple(self.fwd_pos))    
...
class DropSubgoal(Subgoal):
    ...
    def as_language(self):
        text =  "drop the " + self.bot.mission.carrying.color + " " + self.bot.mission.carrying.type
        return LanguageObj(text=text, obj_type=self.bot.mission.carrying.type, 
                           obj_color=self.bot.mission.carrying.color, subgoal_type=type(self), 
                           id=self.id, pos=tuple(self.fwd_pos))
...
class PickupSubgoal(Subgoal):
    ...
    def as_language(self):
        text = "pick up the " + self.fwd_cell.color + " " + self.fwd_cell.type
        return LanguageObj(text=text, obj_type=self.fwd_cell.type, 
                           obj_color=self.fwd_cell.color, subgoal_type=type(self), 
                           id=self.id, pos=tuple(self.fwd_pos))
...
class GoNextToSubgoal(Subgoal):
    ...
    def replan_before_action(self):
        ...
        # Save the target pos for language
    ...
    def as_language(self):
        if self.is_exploratory():
            # We can say things like "explore the red door" to state that we are just looking for it
            # instead of it being part of the objective.
            text = "explore "
            if isinstance(self.datum, ObjDesc) or isinstance(self.datum, WorldObj):
                obj_type = self.datum.type if self.datum.type else None
                obj_color = self.datum.color if self.datum.color else None
                if obj_color:
                    text += "the " + obj_color + " " + obj_type
                elif self.bot.mission.grid.get(*self._target_pos) and \
                    self.bot.mission.grid.get(*self._target_pos).color:
                    text += "the " + self.bot.mission.grid.get(*self._target_pos).color + " " + obj_type
                else:
                    text += "a " + obj_type
            else:
                obj_type = None
                obj_color = None
                # TODO: Compute the direction relative to the agents current position when this goal was planned.

            # TODO: determine how to give explore instructions.
            return LanguageObj(text=text, obj_type=obj_type, 
                           obj_color=obj_color, subgoal_type=type(self), 
                           id=self.id, pos=self._target_pos)
...
class ExploreSubgoal(Subgoal):
    ...
    def as_language(self):
        ...
        return LanguageObj(text="explore", obj_type=None, 
                           obj_color=None, subgoal_type=type(self), 
                           id=self.id, pos=None)
